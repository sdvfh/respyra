...
"""
This code is for the PsySym/relevance_model/main.py
...
TOKEN = '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'


def main(args):
    tokenizer = AutoTokenizer.from_pretrained(args.model_type, truncation=True, use_auth_token=TOKEN)
    data_module = MultiDiseaseDataModule(args.bs, args.input_dir, tokenizer, args.max_len, args.uncertain,
                                         args.bal_sample, args.control_ratio)
    ...
    trainer = pl.Trainer(callbacks=[early_stop_callback, checkpoint_callback, pos_callback], val_check_interval=1.0,
                         max_epochs=100, min_epochs=1, accumulate_grad_batches=args.accumulation,
                         gradient_clip_val=args.gradient_clip_val, deterministic=True, log_every_n_steps=10)
